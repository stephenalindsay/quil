...
(defn- current-surface [] *pjs*)

(def current-applet current-surface)
...
(defn set-state!
  [& state-vals]
  (if-not @*state*
    (reset! *state* (apply hash-map state-vals))
    (.log js/console "State already set!")))
...
(def ^{:private true}
  render-modes {:p2d (.-P2D (.-prototype js/Processing))
                :p3d (.-P3D (.-prototype js/Processing))
                :java2d (.-JAVA2D (.-prototype js/Processing))
                :opengl (.-OPENGL (.-prototype js/Processing))
                :pdf (.-PDF (.-prototype js/Processing))
                :dxf (.-DXF (.-prototype js/Processing))})
...
(def ^{:private true}
     shape-modes {:points (.-POINTS (.-prototype js/Processing))
                  :lines (.-LINES (.-prototype js/Processing))
                  :triangles (.-TRIANGLES (.-prototype js/Processing))
                  :triangle-strip (.-TRIANGLE_STRIP (.-prototype js/Processing))
                  :triangle-fan (.-TRIANGLE_FAN (.-prototype js/Processing))
                  :quads (.-QUADS (.-prototype js/Processing))
                  :quad-strip (.-QUAD_STRIP (.-prototype js/Processing))})
...
(def ^{:private true}
  blend-modes {:blend (.-BLEND (.-prototype js/Processing))
               :add (.-ADD (.-prototype js/Processing))
               :subtract (.-SUBTRACT (.-prototype js/Processing))
               :darkest (.-DARKEST (.-prototype js/Processing))
               :lightest (.-LIGHTEST (.-prototype js/Processing))
               :difference (.-DIFFERENCE (.-prototype js/Processing))
               :exclusion (.-EXCLUSION (.-prototype js/Processing))
               :multiply (.-MULTIPLY (.-prototype js/Processing))
               :screen (.-SCREEN (.-prototype js/Processing))
               :overlay (.-OVERLAY (.-prototype js/Processing))
               :hard-light (.-HARD_LIGHT (.-prototype js/Processing))
               :soft-light (.-SOFT_LIGHT (.-prototype js/Processing))
               :dodge (.-DODGE (.-prototype js/Processing))
               :burn (.-BURN (.-prototype js/Processing))})
...
(def ^{:private true}
  cursor-modes {:arrow (.-ARROW (.-prototype js/Processing))
                :cross (.-CROSS (.-prototype js/Processing))
                :hand (.-HAND (.-prototype js/Processing))
                :move (.-MOVE (.-prototype js/Processing))
                :text (.-TEXT (.-prototype js/Processing))
                :wait (.-WAIT (.-prototype js/Processing))})
...
(def ^{:private true}
     ellipse-modes   {:center (.-CENTER (.-prototype js/Processing))
                      :radius (.-RADIUS (.-prototype js/Processing))
                      :corner (.-CORNER (.-prototype js/Processing))
                      :corners (.-CORNERS (.-prototype js/Processing))})
...
(defn
  ...
  ([mode]
     (when-not (= :close mode)
       (throw (js/Error. (str "Unknown mode value: " mode ". Expected :close"))))
     (.endShape (current-applet) (.-CLOSE (.-prototype js/Processing))))) ;PApplet/CLOSE)))
...
(def ^{:private true}
  filter-modes {:threshold (.-THRESHOLD (.-prototype js/Processing))
                :gray (.-GRAY (.-prototype js/Processing))
                :invert (.-INVERT (.-prototype js/Processing))
                :posterize (.-POSTERIZE (.-prototype js/Processing))
                :blur (.-BLUR (.-prototype js/Processing))
                :opaque (.-OPAQUE (.-prototype js/Processing))
                :erode (.-ERODE (.-prototype js/Processing))
                :dilate (.-DILATE (.-prototype js/Processing))})
...
(def ^{:private true}
  hint-options {:enable-opengl-4x-smooth (.-ENABLE_OPENGL_4X_SMOOTH (.-prototype js/Processing))
                :enable-opengl-2x-smooth (.-ENABLE_OPENGL_2X_SMOOTH (.-prototype js/Processing))
                :enable-native-fonts (.-ENABLE_NATIVE_FONTS (.-prototype js/Processing))
                :disable-native-fonts (.-DISABLE_NATIVE_FONTS (.-prototype js/Processing))
                :enable-depth-test (.-ENABLE_DEPTH_TEST (.-prototype js/Processing))
                :disable-depth-test (.-DISABLE_DEPTH_TEST (.-prototype js/Processing))
                :enable-depth-sort (.-ENABLE_DEPTH_SORT (.-prototype js/Processing))
                :disable-depth-sort (.-DISABLE_DEPTH_SORT (.-prototype js/Processing))
                :disable-opengl-error-report (.-DISABLE_OPENGL_ERROR_REPORT (.-prototype js/Processing))
                :enable-opengl-error-report (.-ENABLE_OPENGL_ERROR_REPORT (.-prototype js/Processing))
                :enable-accurate-textures (.-ENABLE_ACCURATE_TEXTURES (.-prototype js/Processing))
                :disable-accurate-textures (.-DISABLE_ACCURATE_TEXTURES (.-prototype js/Processing))
                :disable-depth-mask (.-DISABLE_DEPTH_MASK (.-prototype js/Processing))
                :enable-depth-mask (.-ENABLE_DEPTH_MASK (.-prototype js/Processing))})
...
(def ^{:private true}
  image-modes {:corner (.-CORNER (.-prototype js/Processing))
               :corners (.-CORNERS (.-prototype js/Processing))
               :center (.-CENTER (.-prototype js/Processing))})
...
(defn
  ...
  (.pow *pjs* (float num) (float exponent))
  )
...
(def ^{:private true}
  rect-modes {:corner (.-CORNER (.-prototype js/Processing))
              :corners (.-CORNERS (.-prototype js/Processing))
              :center (.-CENTER (.-prototype js/Processing))
              :radius (.-RADIUS (.-prototype js/Processing))})
...
(def ^{:private true}
  p-shape-modes {:corner (.-CORNER (.-prototype js/Processing))
                 :corners (.-CORNERS (.-prototype js/Processing))
                 :center (.-CENTER (.-prototype js/Processing))})
...
(def ^{:private true}
  stroke-cap-modes {:square  (.-SQUARE (.-prototype js/Processing))
                    :round   (.-ROUND (.-prototype js/Processing))
                    :project (.-PROJECT (.-prototype js/Processing))
                    :model   (.-MODEL (.-prototype js/Processing))})
...
(def ^{:private true}
  stroke-join-modes {:miter (.-MITER (.-prototype js/Processing))
                     :bevel (.-BEVEL (.-prototype js/Processing))
                     :round (.-ROUND (.-prototype js/Processing))})
...
(def ^{:private true}
  horizontal-alignment-modes {:left (.-LEFT (.-prototype js/Processing))
                              :center (.-CENTER (.-prototype js/Processing))
                              :right (.-RIGHT (.-prototype js/Processing))})

(def ^{:private true}
  vertical-alignment-modes {:top (.-TOP (.-prototype js/Processing))
                            :bottom (.-BOTTOM (.-prototype js/Processing))
                            :center (.-CENTER (.-prototype js/Processing))
                            :baseline (.-BASELINE (.-prototype js/Processing))})
...
(def ^{:private true}
  text-modes {:model (.-MODEL (.-prototype js/Processing))
              :shape (.-SHAPE (.-prototype js/Processing))
              :screen (.-SCREEN (.-prototype js/Processing))})
...
(def ^{:private true}
  texture-modes {:image (.-IMAGE (.-prototype js/Processing))
                 :normalized (.-NORMALIZED (.-prototype js/Processing))})
...
(defn defsketch-old
  ...
  (if-let [canvas (.getElementById js/document id)]
    (let [{:keys [title setup draw size renderer]} (apply hash-map args)
          [width height] size
          state  (atom nil)
      ...
      (set! (.-draw pjs) (fn []
                           (binding [*pjs* pjs
                             ...
                             (draw))))
      (.setup pjs))
    (.log js/console "No canvas element with id : " id)))
