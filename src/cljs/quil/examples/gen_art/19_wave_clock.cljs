(ns quil.examples.gen-art.wave-clock
  (:use-macros [quiljs.macros :only [defsketch]])
  (:use [quil.core :only [background stroke-weight smooth no-fill stroke radians random PI
                          cos sin line width height noise frame-rate set-state! state]]
