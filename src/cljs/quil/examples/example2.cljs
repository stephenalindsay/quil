(ns example2
  (:use [quil.core :only [background-float stroke-float fill-float
                          begin-shape vertex end-shape display-filter
                          smooth no-stroke fill frame-rate defsketch QUARTER-PI]])
  (:use-macros [quiljs.macros :only [defsketch with-translation with-rotation]]))
